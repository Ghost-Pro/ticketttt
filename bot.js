...
client.on("message", (message) => {
   ...
   if (message.content.startsWith("$new")) {  
        ...
        message.guild.createChannel(`ticket-${message.author.username}`, "text").then(c => {
            ...
            const embed = new Discord.RichEmbed()
                .setColor(0xCF40FA)
                .addField(`**Welcome To Deviants Server Please Dont Mention In Room** ${message.author.username}!`, `**Has Been Created ticket** :white_check_mark:`)
                .setTimestamp();
            c.send({
                embed: embed
            });
  ...
  if (message.content.startsWith("$close")) {
       ...
       message.channel.send(`**Type $Close For Closing**`)
           .then((m) => {
